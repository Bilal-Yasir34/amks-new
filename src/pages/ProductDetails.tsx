...
<<<<<<< HEAD
      ...
      if (p.category_id) {
        ...
        if (relData) relProducts = relData as Product[];
      }

      if (relProducts.length < 4) {
        const excludeIds = [p.id, ...relProducts.map((rp) => rp.id)];
        const { data: otherData } = await supabase
          .from('products')
          .select('*, category:categories(*)')
          .eq('status', 'active')
          .not('id', 'in', `(${excludeIds.join(',')})`)
          .limit(4 - relProducts.length);
        if (otherData) {
          relProducts = [...relProducts, ...(otherData as Product[])];
        }
      }
