...
function BannerForm({ banner, onClose, onSaved }: { banner: HeroBanner | null; onClose: () => void; onSaved: () => void }) {
  ...
  const handleUpload = async (file: File) => {
    try {
      ...
      const mediaRes = await supabase.from('media_assets').insert({ url: data.publicUrl, file_name: file.name, file_path: fileName, content_type: file.type, file_size: file.size });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (banner) {
        const { data, error } = await supabase.from('hero_banners').update(payload).eq('id', banner.id).select();
