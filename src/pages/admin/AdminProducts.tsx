...
<<<<<<< HEAD
  ...
  const insertFormat = (tagOpen: string, tagClose: string) => {
    ...
    const text = form.long_description;
    
    const selectedText = text.substring(start, end);
    const replacement = tagOpen + selectedText + tagClose;
    
    const newValue = text.substring(0, start) + replacement + text.substring(end);
    setForm({ ...form, long_description: newValue });
...
<<<<<<< HEAD
              <div>
                ...
                <div className="border border-ink-200 bg-white">
                  <div className="flex items-center gap-1 p-2 border-b border-ink-100 bg-stone-light/50">
                    <button
                      type="button"
                      onMouseDown={(e) => { e.preventDefault(); insertFormat('<strong>', '</strong>'); }}
                      className="px-2 py-1 text-xs font-bold border border-ink-200 bg-white hover:bg-ink-100 text-ink-900 transition-colors"
                      title="Bold"
                    >
                      B
                    </button>
                    <button
                      type="button"
                      onMouseDown={(e) => { e.preventDefault(); insertFormat('<em>', '</em>'); }}
                      className="px-2 py-1 text-xs italic border border-ink-200 bg-white hover:bg-ink-100 text-ink-900 transition-colors"
                      title="Italic"
                    >
                      I
                    </button>
                    <button
                      ...
                      className="px-2 py-1 text-xs underline border border-ink-200 bg-white hover:bg-ink-100 text-ink-900 transition-colors"
                      title="Underline"
                    >
                      U
                    </button>
                    <button
                      type="button"
                      onMouseDown={(e) => { e.preventDefault(); insertFormat('<p>', '</p>'); }}
                      className="px-2 py-1 text-[10px] font-mono border border-ink-200 bg-white hover:bg-ink-100 text-ink-900 transition-colors"
                      title="Paragraph"
                    >
                      Para
                    </button>
                    <button
                      type="button"
                      onMouseDown={(e) => { e.preventDefault(); insertFormat('<br />', ''); }}
                      className="px-2 py-1 text-[10px] font-mono border border-ink-200 bg-white hover:bg-ink-100 text-ink-900 transition-colors"
                      title="Line Break"
                    >
                      Br
                  ...
                  <textarea
                    id="long_description_textarea"
                    rows={4}
                    value={form.long_description}
                    onChange={(e) => setForm({ ...form, long_description: e.target.value })}
                    className="w-full p-3 text-sm bg-transparent outline-none focus:ring-0 resize-none border-none"
                    placeholder="Enter long description. Use toolbar buttons to apply styles."
                  />
                </div>
              </div>
=======
...
<<<<<<< HEAD
                  {!product ? (
                    ...
                    <div className="border border-ink-100 p-6 space-y-4 bg-stone-light/50 shadow-sm">
                      <div className="flex justify-between items-center">
                        <div>
                          <h3 className="text-sm font-medium">Configure Product Variants</h3>
                          <p className="text-xs text-ink-400 mt-1">
                            Current setup: {variantGroups.length} groups, {combinations.length} active combinations.
                          </p>
                        </div>
                        <button
                          type="button"
                        ...
                        >
                          Manage Variants, Images & Pricing
                        </button>
                      </div>
                    </div>
=======
...
interface VariantsModalProps {
  ...
  setCombinations: React.Dispatch<React.SetStateAction<ProductVariantCombination[]>>;
  variantImages: ProductVariantImage[];
  setVariantImages: React.Dispatch<React.SetStateAction<ProductVariantImage[]>>;
...
function VariantsModal({
  ...
  const [activeValueImages, setActiveValueImages] = useState<ProductVariantValue | null>(null);
  ...
  const addGroup = async () => {
    ...
    const { data } = await supabase.from('product_variant_groups').insert({ product_id: product.id, name, sort_order: variantGroups.length }).select('*, product_variant_values(*)').single();
    if (data) {
      setVariantGroups([...variantGroups, data as ProductVariantGroup]);
      setNewGroupName('');
  ...
  const addValue = async (groupId: string) => {
    const value = newValueNames[groupId]?.trim();
    if (!value) return;
    const group = variantGroups.find((g) => g.id === groupId);
    const sortOrder = group?.product_variant_values?.length || 0;
    const { data } = await supabase.from('product_variant_values').insert({ group_id: groupId, value, sort_order: sortOrder }).select().single();
  ...
  const deleteValue = async (groupId: string, valueId: string) => {
    await supabase.from('product_variant_values').delete().eq('id', valueId);
    setVariantGroups(variantGroups.map((g) => g.id === groupId ? { ...g, product_variant_values: (g.product_variant_values || []).filter((v) => v.id !== valueId) } : g));
    setVariantImages(variantImages.filter(vi => vi.variant_value_id !== valueId));
    toast.success('Value removed.');
  };

  const handleVariantImageUpload = async (file: File) => {
    if (!activeValueImages) return;
    setUploading(true);
    try {
      const url = await uploadImage(file);
      const valImages = variantImages.filter(vi => vi.variant_value_id === activeValueImages.id);
      const { data } = await supabase.from('product_variant_images').insert({
        product_id: product.id,
        variant_value_id: activeValueImages.id,
        image_url: url,
        sort_order: valImages.length
      }).select().single();
      if (data) {
        setVariantImages([...variantImages, data as ProductVariantImage]);
  ...
  const deleteVariantImage = async (id: string) => {
    await supabase.from('product_variant_images').delete().eq('id', id);
    setVariantImages(variantImages.filter(vi => vi.id !== id));
  ...
  const generateCombos = async () => {
    ...
    const rows = combos.map((combo) => ({
      product_id: product.id,
      combination_key: combo.join('|'),
      regular_price: parseFloat(product.regular_price as any) || 0,
  ...
  const updateComboField = (id: string, field: string, value: any) => {
    setCombinations(combinations.map((c) => c.id === id ? { ...c, [field]: value === '' ? null : value } : c));
  };

  const saveCombo = async (c: ProductVariantCombination) => {
    await supabase.from('product_variant_combinations').update({
      sku: c.sku,
      regular_price: c.regular_price,
      sale_price: c.sale_price,
  ...
  return (
    <div className="fixed inset-0 bg-ink-900/60 z-[100] backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-6xl h-[90vh] flex flex-col shadow-2xl relative border border-ink-100">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-ink-100">
          <div>
            <h2 className="font-display text-2xl font-light">Manage Product Variants</h2>
            <p className="text-xs text-ink-400 mt-1">Configure variant attributes, upload specific images, and manage combination pricing.</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-stone-light"><X className="w-5 h-5" /></button>
        </div>

        {/* Content */}
        <div className="flex-1 flex overflow-hidden">
          {/* Left Column: Groups & Values */}
          <div className="w-5/12 border-r border-ink-100 p-6 overflow-y-auto space-y-6">
            <h3 className="text-xs tracking-widest uppercase font-semibold text-ink-400">Variant Groups</h3>
            ...
            <div className="flex gap-2">
              ...
              />
              <button onClick={addGroup} className="btn-primary !py-2 !px-4 text-xs whitespace-nowrap"><Plus className="w-4 h-4 inline mr-1" />Add Group</button>
            </div>

            {/* List Groups */}
            <div className="space-y-4">
              {variantGroups.map((g) => (
                <div key={g.id} className="border border-ink-100 p-4 space-y-3 bg-stone-light/50">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium">{g.name}</span>
                    <button onClick={() => deleteGroup(g.id)} className="text-xs text-red-500 hover:text-red-700">Delete Group</button>
                  </div>

                  {/* Values List */}
                  <div className="flex flex-wrap gap-2">
                    {(g.product_variant_values || []).map((val) => {
                      const imageCount = variantImages.filter(vi => vi.variant_value_id === val.id).length;
                      return (
                        <div key={val.id} className="flex items-center gap-1 bg-white border border-ink-100 pl-2 pr-1 py-1 text-xs">
                          <span>{val.value}</span>
                          <button
                            type="button"
                            onClick={() => setActiveValueImages(val)}
                            className={`ml-1 px-1.5 py-0.5 rounded text-[10px] flex items-center gap-0.5 ${imageCount > 0 ? 'bg-ink-900 text-white' : 'bg-ink-100 text-ink-600 hover:bg-ink-200'}`}
                          >
                            🖼️ {imageCount}
                          </button>
                          <button type="button" onClick={() => deleteValue(g.id, val.id)} className="ml-1 text-ink-300 hover:text-red-500"><X className="w-3.5 h-3.5" /></button>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-ink-100/50">
                    <input
                      type="text"
                      placeholder={`Add value to ${g.name}`}
                      value={newValueNames[g.id] || ''}
                      onChange={(e) => setNewValueNames({ ...newValueNames, [g.id]: e.target.value })}
                      className="input-field flex-1 !py-1 text-xs"
                    />
                    <button type="button" onClick={() => addValue(g.id)} className="btn-outline !py-1 !px-3 text-[10px] whitespace-nowrap">Add Value</button>
                  </div>
                </div>
              ))}
              {variantGroups.length === 0 && <p className="text-xs text-ink-400">No variant groups defined yet. Add one above.</p>}
            </div>
          </div>

          {/* Right Column: Combinations */}
          <div className="w-7/12 p-6 overflow-y-auto flex flex-col space-y-4 bg-stone-light/10">
            <div className="flex items-center justify-between">
              <h3 className="text-xs tracking-widest uppercase font-semibold text-ink-400">Combinations & Pricing</h3>
              {variantGroups.some(g => (g.product_variant_values || []).length > 0) && (
                <button type="button" onClick={generateCombos} className="text-xs btn-outline !py-1.5 !px-3">Regenerate Combinations</button>
              )}
            </div>
            ...
            {combinations.length > 0 ? (
              <div className="space-y-3">
                {combinations.map((c) => (
                  <div key={c.id} className="bg-white border border-ink-100 p-4 space-y-3 shadow-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold tracking-wider text-ink-600">{c.combination_key.replace(/\|/g, ' / ')}</span>
                      <span className="text-[10px] bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded font-mono">{c.sku || 'No SKU'}</span>
                    </div>

                    <div className="grid grid-cols-4 gap-3">
                      <div>
                        <label className="text-[10px] text-ink-400 block mb-1">SKU</label>
                        <input
                          type="text"
                          value={c.sku || ''}
                          onChange={(e) => updateComboField(c.id, 'sku', e.target.value)}
                          className="input-field !py-1 text-xs"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] text-ink-400 block mb-1">Regular Price *</label>
                        <input
                          type="number"
                          value={c.regular_price}
                          onChange={(e) => updateComboField(c.id, 'regular_price', parseFloat(e.target.value) || 0)}
                          className="input-field !py-1 text-xs"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] text-ink-400 block mb-1">Sale Price</label>
                        <input
                          type="number"
                          value={c.sale_price ?? ''}
                          onChange={(e) => updateComboField(c.id, 'sale_price', e.target.value === '' ? null : parseFloat(e.target.value))}
                          className="input-field !py-1 text-xs"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] text-ink-400 block mb-1">Stock</label>
                        <input
                          type="number"
                          value={c.stock_quantity}
                          onChange={(e) => updateComboField(c.id, 'stock_quantity', parseInt(e.target.value) || 0)}
                          className="input-field !py-1 text-xs"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end pt-1">
                      <button type="button" onClick={() => saveCombo(c)} className="btn-primary !py-1 !px-4 text-[10px]">Save Combination</button>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="flex-1 border border-dashed border-ink-200 flex flex-col items-center justify-center p-8 text-center bg-white min-h-[250px]">
                <Layers className="w-8 h-8 text-ink-300 mb-2" />
                <p className="text-xs text-ink-400">No active combinations generated.</p>
                <p className="text-[10px] text-ink-300 mt-1">Configure variant groups on the left, then click Regenerate Combinations.</p>
              </div>
            )}
          </div>
        </div>

        {/* Sub-panel overlay: Images Management */}
        {activeValueImages && (
          <div className="absolute inset-0 bg-ink-900/60 z-[110] backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl h-[70vh] flex flex-col shadow-2xl border border-ink-200">
              <div className="flex items-center justify-between p-6 border-b border-ink-100">
                <div>
                  <h3 className="font-display text-lg font-light">Manage Images for {activeValueImages.value}</h3>
                  <p className="text-xs text-ink-400 mt-0.5">Upload specific images that display on the site when this variant is selected.</p>
                </div>
                <button onClick={() => setActiveValueImages(null)} className="p-2 hover:bg-stone-light"><X className="w-5 h-5" /></button>
              </div>

              {/* Variant Images list */}
              <div className="flex-1 p-6 overflow-y-auto space-y-6">
                <div className="grid grid-cols-4 gap-4">
                  {variantImages.filter(vi => vi.variant_value_id === activeValueImages.id).map((img) => (
                    <div key={img.id} className="relative group aspect-square border border-ink-100 bg-stone-light overflow-hidden">
                ...
                <div className="border-t border-ink-100 pt-6">
                  <label className="text-xs font-semibold tracking-wider text-ink-400 uppercase block mb-3">Upload Image</label>
                  <label className={`border-2 border-dashed border-ink-200 hover:border-ink-900 rounded p-8 text-center flex flex-col items-center justify-center cursor-pointer transition-colors ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                    <Plus className="w-6 h-6 text-ink-400 mb-2" />
                    <span className="text-xs text-ink-600 font-medium">Click to select image</span>
                    <span className="text-[10px] text-ink-400 mt-1">PNG, JPG or WEBP formats</span>
                    <input
                      type="file"
                      accept="image/*"
                      className="hidden"
                      disabled={uploading}
                      onChange={(e) => e.target.files?.[0] && handleVariantImageUpload(e.target.files[0])}
                    />
                  </label>
                  {uploading && <p className="text-xs text-ink-400 mt-2 text-center animate-pulse">Uploading image to storage...</p>}
                </div>
              </div>

              <div className="p-4 border-t border-ink-100 flex justify-end bg-stone-light/50">
                <button onClick={() => setActiveValueImages(null)} className="btn-outline !py-1.5 !px-5 text-xs">Done</button>
              </div>
            </div>
          </div>
        )}
